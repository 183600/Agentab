...
export function makeAccessible(element, options = {}) {
  const { role, label, description, tabindex, hidden } = options;
...
export function createAccessibleButton(options = {}) {
  const { text, icon, label, pressed, expanded, controls, haspopup, onClick } = options;
...
export function createAccessibleDialog(options = {}) {
  const { title, description, labelledBy, describedBy } = options;
