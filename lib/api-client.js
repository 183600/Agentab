...
import { ApiError, TimeoutError, AbortError, ConfigurationError } from './errors.js';
...
export class LlmApiClient {
  ...
  async getConfig() {
    ...
    if (!apiKey) {
      throw new ConfigurationError('API key not configured. Please set it in the extension settings.', 'apiKey');
    }
    ...
    for (let attempt = 1; attempt <= retries; attempt++) {
      ...
      try {
        ...
        if (attempt < retries) {
          ...
          apiLogger.warn(`API request attempt ${attempt} failed, retrying in ${delay}ms...`, { error: error.message });
  ...
  async listModels() {
    try {
      ...
      apiLogger.error('Failed to list models', { error: error.message });
      return [];
    }
  }
}
