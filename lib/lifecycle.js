...
export class ServiceWorkerLifecycle {
  ...
  handleKeepAlive() {
    const now = Date.now();
    const taskDuration = this.taskState.taskStartTime ? now - this.taskState.taskStartTime : 0;

    // Check if we should stop keep-alive
    if (this.taskState.activeTaskId && taskDuration > this.config.maxKeepAliveDuration) {
      if (this.config.debug) {
  ...
  async wakeUp() {
    ...
    if (this.config.debug) {
      logger.debug('Lifecycle woke up');
    }
  ...
  getStatus() {
    return {
      ...
      taskDuration: this.taskState.taskStartTime ? Date.now() - this.taskState.taskStartTime : 0,
