  ...
  async flush() {
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (this.config.debug) {
          logger.debug('Monitoring flushed events', { count: events.length });
        }
      } else {
        // Re-queue on failure
      ...
      logger.warn('Failed to flush monitoring events', { error: e.message });
    }
  }
...
export const captureMessage = (message, level, hints) =>
  getMonitoring().captureMessage(message, level, hints);
