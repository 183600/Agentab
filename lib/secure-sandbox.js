...
/**
 * Check if running in browser environment with DOM support
 * @returns {boolean}
 */
...
export async function secureExecute(code, context = {}) {
  ...
  // Use executeWithFunction in non-browser environments (e.g., Node.js tests)
  const executeMethod = hasDomSupport() ? sandbox.execute : sandbox.executeWithFunction;
  const result = await executeMethod.call(sandbox, code, context);
