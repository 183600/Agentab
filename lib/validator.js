...
export class InputValidator {
  ...
  static validatePrompt(prompt, options = {}) {
    const { minLength = 1, maxLength = 10000, allowHtml = false, allowNewlines = true } = options;
  ...
  static validateCode(code, options = {}) {
    const { minLength = 1, maxLength = 100000 } = options;
    ...
    for (const char of value) {
      if (brackets[char]) {
        ...
        if (brackets[last] !== char) {
          ...
          logger.warn('Potentially unbalanced brackets in code');
          break;
        }
      }
    }
  ...
  static validateTask(task) {
    ...
    const contentResult =
      task.type === 'prompt' ? this.validatePrompt(task.content) : this.validateCode(task.content);
