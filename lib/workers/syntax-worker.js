...
const TOKEN_PATTERNS = {
  javascript: [
    ...
    { pattern: /\b(const|let|var|function|return|if|else|for|while|do|switch|case|break|continue|new|class|extends|import|export|from|default|async|await|try|catch|finally|throw|typeof|instanceof|in|of)\b/g, className: 'keyword' },
    ...
    { pattern: /\b(console|document|window|Math|JSON|Array|Object|String|Number|Boolean|Date|RegExp|Promise|Map|Set)\b/g, className: 'builtin' },
  ...
  python: [
    ...
    { pattern: /("(?:[^"\\]|\\.)*"|'(?:[^'\\]|\\.)*')/g, className: 'string' },
    { pattern: /\b(def|class|if|elif|else|for|while|try|except|finally|with|as|import|from|return|yield|raise|pass|break|continue|and|or|not|in|is|lambda|True|False|None)\b/g, className: 'keyword' },
    { pattern: /\b(\d+\.?\d*)\b/g, className: 'number' },
    { pattern: /\b([A-Z][a-zA-Z0-9]*)\b/g, className: 'class-name' },
  ...
  bash: [
    { pattern: /(#.*$)/gm, className: 'comment' },
    { pattern: /("(?:[^"\\]|\\.)*"|'(?:[^'\\]|\\.)*')/g, className: 'string' },
    { pattern: /\b(if|then|else|fi|for|while|do|done|case|esac|function|return|exit|export|source|echo|printf|read|cd|pwd|mkdir|rm|cp|mv|ls|cat|grep|sed|awk|find|xargs|chmod|chown)\b/g, className: 'keyword' },
...
function analyzeCode(code, lang = 'javascript') {
  ...
  if (lang === 'javascript' || lang === 'typescript') {
    ...
    const funcRegex = /(?:function\s+(\w+)|(?:const|let|var)\s+(\w+)\s*=\s*(?:async\s*)?\(|(\w+)\s*:\s*(?:async\s*)?function|(?:async\s+)?(\w+)\s*\([^)]*\)\s*\{)/g;
    let match;
