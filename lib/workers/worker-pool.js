  ...
  filterItems: (items, conditions) => {
    return items.filter(item => {
      for (const { field, operator, value } of conditions) {
        ...
        switch (operator) {
          case 'eq': if (itemValue !== value) return false; break;
          case 'neq': if (itemValue === value) return false; break;
          case 'gt': if (!(itemValue > value)) return false; break;
          case 'gte': if (!(itemValue >= value)) return false; break;
          case 'lt': if (!(itemValue < value)) return false; break;
          case 'lte': if (!(itemValue <= value)) return false; break;
          case 'contains': if (!String(itemValue).includes(value)) return false; break;
          case 'startsWith': if (!String(itemValue).startsWith(value)) return false; break;
          case 'endsWith': if (!String(itemValue).endsWith(value)) return false; break;
          case 'matches': if (!new RegExp(value).test(String(itemValue))) return false; break;
          case 'in': if (!value.includes(itemValue)) return false; break;
          case 'notIn': if (value.includes(itemValue)) return false; break;
        }
      }
      return true;
    });
